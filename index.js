...
const { db_server } = require("./db_server.js");

//const { httpDB, PORT } = require("dotenv").config({ path: "Vote_Bot/.env" }).parsed;
const { PORT, bot_token, admin, twinkByAdmin } =
...
const answerProcessing = async function (ctx) {
  ...
  if (poll_id === activePolls[0].poll_id) {
    ...
    if (options[0] === 1) {
      await sendMessage(player, texts.cantToday, buttons.deleteThisMessage);
      return await player_voteAPI.update(player, { ready_to_play: false });
    }
...
const devFun = async function () {
  setTimeout(async () => {
    //await sendMessage(admin, texts.cantToday, buttons.deleteThisMessage);
