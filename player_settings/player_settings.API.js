...
class player_settingsAPI {
    ...
    async getAll(is_personal_result = true) {
        const receivedGet = await axios.get(`http://localhost:${PORT}/player_settings/?is_personal_result=${is_personal_result}`)
        return receivedGet
    }

    //get player for showing current settings
    async get(id) {
        const receivedGet = await axios.get(`http://localhost:${PORT}/player_settings/${id}`)
        return receivedGet
    }
    ...
    async update(id, is_personal_result = true, newData) {
        ...
        const receivedPut = await axios.put(`http://localhost:${PORT}/player_settings/${id}`, newData)
        return receivedPut
    }

    //create settings for new player
    async create(player_id) {
        const receivedPost = await axios.post(`http://localhost:${PORT}/player_settings/`, {player_id})
        return receivedPost
    }
}
