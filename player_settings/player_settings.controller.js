...
class Player_settingsController {
    async getAllPlayer_settings(req, res) {
        ...
        const player_settings = await db.query(`select * from player_settings where (is_personal_result = ${is_personal_result} AND enabled = true);`)
    ...
    async getPlayer(req, res) {   
        ...
        const player_settings = await db.query(`select * FROM player_settings WHERE (player_id = ${player_id}) ORDER BY is_personal_result desc`)
